...
function [t, Sol] = bud_sage_microparticle_dyn(alpha, mesh, lambda, fc, phi1, alpha0, k0, dk, dkd, dkG, P, gamma, C0, D0, D1, R0, tt, initSol)

t=alpha*mesh;   % area mesh points

% declare and assign global variables to be used in nested functions
global a acap f gt a0 ac iSol lam c0 d0 d1 g delta deltad delkG p pw wd phi
...
options = bvpset('NMax', 100*length(t), 'RelTol', 1e-4);
...
%file_name=['/Users/ariji/Documents/MATLAB/sage/shape' sprintf('%f',tt) 'a' sprintf('%f',k0)  '.txt'];
%fileID4=fopen(file_name,'w+');

%file_name=['/Users/ariji/Documents/MATLAB/sage/energy' sprintf('%f',tt) 'a' sprintf('%f',k0)  '.txt'];
...
c = 0.5*c0*(tanh(g*(t+ a0)) - tanh(g*(t - a0)));
m= 0.5*d0*(tanh(g*(t +a0)) - tanh(g*(t - a0)))-0.5*d0*(tanh(g*(t+ acap)) - tanh(g*(t - acap))); 
phit=interp1(gt,phi,t);
fcl=f*phit; 
...
for i=1:2*N-1
 ...
 %  fprintf(fileID4,'%12.8f \t %12.8f \t %12.8f \t %12.8f \t %12.8f \t %12.8f \t %12.8f',xx(i),yy(i),cc(i),ww(i),HH(i),llam(i),ffcl(i)); 
  % fprintf(fileID4,'\n');
...
%parameters
global a acap f c0 d0 d1 a0 ac g delta deltad delkG p pw wd phi gt

% spontaneous curvature
%c = 0.5*c0*(1 - tanh(g*(t - a0)));
%phi=0.5*(tanh(g*(t +a0)) - tanh(g*(t - a0)));
phit = interp1(gt,phi,t);
...
dM = 0.5*d0*g*(tanh(g*(t - a0))^2 - tanh(g*(t + a0))^2)-0.5*d0*g*(tanh(g*(t - acap))^2 - tanh(g*(t + acap))^2);
...
dXdt = [cos(X(3))/X(1)
        ...
        (p-fcl*X(2)*cos(X(3))-2*k1*b*(X(4)-C)*(X(4)^2+(X(4)-sin(X(3))/X(1))^2)+2*X(4)*(k1*b*(X(4)-C)^2+k2*bd*(sin(X(3))/X(1)-X(4)-M)^2+X(6)-2*k2*bd*(sin(X(3))/X(1)-X(4)-M)*(sin(X(3))/X(1)-X(4)))-(k1*db+k2*dbd)*X(5))/(k1*b+k2*bd)%            -((db*2*k1+dbd*k2)/(b*2*k1+bd*k2))*X(5)-2*(X(4))*(X(4)^2+(X(4)-sin(X(3))/X(1))^2)+2*X(4)*((X(4)-C)^2-k2*bd/(2*k1*b+k2*bd)*(sin(X(3))/X(1)-X(4)-M)^2+X(6)+2*k2*bd/(2*k1*b+k2*bd)*(sin(X(3))/X(1)-X(4))*(sin(X(3))/X(1)-X(4)-M)) %- 0   %-2*cos(X(3))/X(1)*(k2/(2*k1)*X(5)/X(1)-k2/k1*X(4)*cos(X(3))/X(1)+k2/k1*sin(X(3))*cos(X(3))/X(1)^2+k2/(2*k1)*dM);
         fcl*X(2)*sin(X(3))/X(1)+2*k1*b*(X(4)-C)*dC+2*k2*bd*(sin(X(3))/X(1)-X(4)-M)*dM-k1*db*(X(4)-C)^2-k2*dbd*(sin(X(3))/X(1)-X(4)-M)^2-dkg*kg*(X(4)^2-(sin(X(3))/X(1)-X(4))^2) %+dbd*k2/(2*k1*b+k2*bd)*(sin(X(3))/X(1)-X(4)-M)^2-2*k2*bd/(2*k1*b+k2*bd)*(sin(X(3))/X(1)-X(4)-M)*dM-(db*2*k1+dbd*k2)/(b*2*k1+bd*k2)*(X(4)-C)^2-dkg*kg/(2*k1+k2)*(X(4)^2-(sin(X(3))/X(1)-X(4))^2)+2*(X(4)-C)*dC
        %0
